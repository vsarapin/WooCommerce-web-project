...
if ( ! class_exists( 'Shop_Elite_About' ) ) {
	...
	class Shop_Elite_About {
		 ...
		 * Single instance.
		 *
		 * @var Shop_Elite_About $instance Instance object.
		 */
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Shop_Elite_About ) ) {
				self::$instance = new Shop_Elite_About;
		...
		public function configure() {
			...
			$this->menu_name     = isset( $this->config['menu_name'] ) ? $this->config['menu_name'] : sprintf( esc_html__( 'About %s', 'shop-elite' ), $this->theme_name );
			$this->page_name     = isset( $this->config['page_name'] ) ? $this->config['page_name'] : sprintf( esc_html__( 'About %s', 'shop-elite' ), $this->theme_name );
			$this->tabs          = isset( $this->config['tabs'] ) ? $this->config['tabs'] : array();
			$this->page_slug     = $this->theme_slug . '-about';
			$this->page_url     = admin_url( 'themes.php?page=' . $this->page_slug );
			$this->notice        = '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s. To fully take advantage of the best our theme can offer please make sure you visit theme info page.', 'shop-elite' ), esc_html( $this->theme_name ) ) . '</p><p><a href="' . esc_url( $this->page_url ) . '" class="button button-primary">' . sprintf( esc_html__( 'Get started with %1$s', 'shop-elite' ), $this->theme_name ) . '</a><a href="#" class="btn-dismiss" data-hello="world" data-userid="' . esc_attr( get_current_user_id() ) . '" data-nonce="' . esc_attr( wp_create_nonce( 'shop_elite_dismiss_nonce' ) ) . '">' . esc_html__( 'Dismiss this notice', 'shop-elite' ) . '</a></p>';
		}
		...
		public function hooks() {
			...
			// Dismiss AJAX.
			add_action( 'wp_ajax_shop_elite_dismiss', array( $this, 'dismiss_callback' ) );
			add_action( 'wp_ajax_nopriv_shop_elite_dismiss', array( $this, 'dismiss_callback' ) );
		}
		...
		public function render_page() {
			?>
			<div class="wrap about-wrap saga-about-wrap">
				...
				<?php endif; ?>
				<a href="https://themesaga.com" target="_blank"><div class="wp-badge"></div></a>
		...
		public function render_current_tab_content() {
			...
			if ( method_exists( $this, $method ) ) {
				...
				printf( esc_html__( '%s() method does not exist.', 'shop-elite' ), $method );
			}
		}
		...
		public function getting_started() {
			...
			?>
			<div class="feature-section saga-section saga-section-getting-started three-col">
				<?php foreach ( $content as $item ) : ?>
					<?php $this->render_grid_item( $item ); ?>
				<?php endforeach; ?>
			</div><!-- .feature-section .saga-section -->
			<?php
		}
		...
		public function support() {
			...
			?>
			<div class="feature-section saga-section saga-section-support three-col">
				<?php foreach ( $content as $item ) : ?>
					<?php $this->render_grid_item( $item ); ?>
				<?php endforeach; ?>
			</div><!-- .feature-section .saga-section -->
		...
		public function useful_plugins() {
			...
			?>
			<div class="saga-section saga-section-useful-plugins">
				...
				<?php if ( true !== $tgmpa_complete ) : ?>
					<a href="<?php echo esc_url( $tgmpa_url ); ?>" class="button button-primary"><?php esc_html_e( 'Manage Plugins', 'shop-elite' ); ?></a>
				<?php endif; ?>
		...
		public function demo_content() {
			...
			?>
			<div class="saga-section saga-section-demo-content">
		...
		public function upgrade_to_pro() {
			...
			?>
			<div class="saga-section saga-section-upgrade-to-pro">
        ...
        public function faq() {
            ...
            ?>
            <div class="saga-section saga-section-faq">
                <?php
                if(is_array($content)){
                    $first = true;
        ...
        public function free_vs_pro() {
            ...
            ?>
            <div class="saga-section saga-section-free-vs-pro">
                <table class="free-vs-pro-table">
                    <thead>
                    <tr>
                        <th></th>
                        <th><?php _e('Free', 'shop-elite')?></th>
                        <th><?php _e('Pro', 'shop-elite')?></th>
                    </tr>
                    </thead>
                    <tbody>
                    <?php
		...
		public function assets( $hook ) {
			...
			if ( in_array( $hook, array( 'themes.php', 'appearance_page_' . $this->page_slug ), true ) ) {
				wp_enqueue_style( 'shop-elite-about', get_template_directory_uri() . '/inc/about/css/about' . $min . '.css', array(), '1.0.0' );
				wp_enqueue_script( 'shop-elite-about', get_template_directory_uri() . '/inc/about/js/about' . $min . '.js', array(), '1.0.0' );
			}
		...
		public function display_admin_notice() {
			...
			$dismiss_status = get_user_meta( $user_id, 'shop_elite_dismiss_status', true );
			?>
			<?php if ( current_user_can( 'edit_theme_options' ) && 'themes' === $screen_id && 1 !== absint( $dismiss_status ) ) : ?>
				<div class="saga-about-notice notice notice-about">
					<?php echo $this->notice; ?>
				</div><!-- .saga-about-notice -->
			<?php endif; ?>
			<?php
		}
		...
		public function dismiss_callback() {
			...
			if ( false === wp_verify_nonce( $wpnonce, 'shop_elite_dismiss_nonce' ) ) {
				wp_send_json( $output );
			}

			update_user_meta( $userid, 'shop_elite_dismiss_status', 1 );
