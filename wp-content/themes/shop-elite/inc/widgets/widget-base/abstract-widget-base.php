 ...
 * @extends  WP_Widget
 */
abstract class Shop_Elite_Widget_Base extends WP_Widget {
    ...
    public function __construct() {
        ...
        $this->attrs = apply_filters( 'shop_elite_widget_base_attr_arr', array(
                'type' => 'text',
                'class' => '',
                'label' => '',
                'css' => '',
        ...
        add_action('admin_enqueue_scripts', array($this,'shop_elite_widget_scripts'));
        /**/
    ...
    public function update( $new_instance, $old_instance ) {
        ...
        foreach ( $this->settings as $key => $setting ) {
             ...
             */
            $instance[ $key ] = apply_filters( 'shop_elite_widget_settings_sanitize_option', $instance[ $key ], $new_instance, $key, $setting );
        }
    ...
    public function form($instance) {
        ...
        foreach ($this->settings as $key => $setting) {
            ...
            switch ( $setting['type'] ) {
                    ...
                    <div class="<?php echo esc_attr( $attr['class'] ); ?>" style="<?php echo esc_attr( $attr['css'] ); ?>">
                        ...
                        <div class="image-field">
                            ...
                            <p>
                                ...
                                <button type="button" class="upload_image_button button" data-uploader-title-txt="<?php esc_attr_e( 'Use Image', 'shop-elite' ); ?>" data-uploader-btn-txt="<?php esc_attr_e( 'Choose an Image', 'shop-elite' ); ?>">
                                    <?php _e( 'Upload/Add image', 'shop-elite' ); ?>
                                </button>
                                <button type="button" class="remove_image_button button" style="<?php echo esc_attr($remove_button_style);?>"><?php _e( 'Remove image', 'shop-elite' ); ?></button>
                            </p>
                        </div>
                    </div>
                    <?php
                ...
                default:
                    do_action( 'shop_elite_widget_field_' . $setting['type'], $key, $value, $setting, $instance );
                    break;
            }
        }
    }
     ...
     * @return void
     */
    public function shop_elite_widget_scripts($hook){
        if ('widgets.php' === $hook) {
            wp_enqueue_style( 'wp-color-picker' );
            wp_enqueue_media();
            wp_enqueue_script('shop_elite_widgets', get_template_directory_uri() . '/inc/widgets/widget-base/js/widgets.js', array('jquery','wp-color-picker'), '1.0.0', true);
        }
    }
