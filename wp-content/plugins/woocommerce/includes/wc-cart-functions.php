...
function wc_add_to_cart_message( $products, $show_qty = false, $return = false ) {
	...
	/* translators: %s: product name */
	$added_text = sprintf( _n( '%s добавлено в корзину.', '%s have been added to your cart.', $count, 'woocommerce' ), wc_format_list_of_items( $titles ) );
	...
	if ( 'yes' === get_option( 'woocommerce_cart_redirect_after_add' ) ) {
		...
		$message = sprintf( '<a href="%s" tabindex="1" class="button wc-forward">%s</a> %s', esc_url( wc_get_page_permalink( 'cart' ) ), esc_html__( 'просмотр', 'woocommerce' ), esc_html( $added_text ) );
	}
