...
abstract class CMB_Field {
	...
	public function display() {
		...
		if ( $this->args['repeatable'] ) {
			...
			<button class="button repeat-field button button-primary"><?php esc_html_e( 'Add Features(unlimited for pro)', 'cmb' ); ?></button>
...
class CMB_Group_Field extends CMB_Field {
	...
	function __construct() {
		...
		if ( ! empty( $this->args['fields'] ) ) {
			foreach ( $this->args['fields'] as $f ) {
				...
				$class = _cmb_field_class_for_type_not_acc( $f['type'] );
				$f['show_label'] = true;
	...
	public function html() {
		...
		<?php if ( $this->args['repeatable'] ) : ?>
			<button class="cmb-delete-field button button-primary" title="Remove field"><span class="cmb-delete-field-icon">&times;</span> Remove </button>
		<?php endif; ?>
